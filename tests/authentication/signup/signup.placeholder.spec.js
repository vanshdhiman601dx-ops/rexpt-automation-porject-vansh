...
import { BusinessCategoryPage } from '../../../pages/onboarding/BusinessCategoryPage.js';
import { BusinessDetailsPage } from '../../../pages/onboarding/BusinessDetailsPage.js';
import { BusinessListingPage } from '../../../pages/onboarding/BusinessListingPage.js';
...
const invalidNameScenarios = [
  ...
  { name: 'Emoji', value: '😊John', mode: 'sanitized', forbidden: /[\p{Emoji_Presentation}\u200d]/u },
  { name: 'Unicode Symbols', value: '♠★✓', mode: 'sanitized', forbidden: /[♠★✓]/ },
  { name: 'Non-English Characters - Chinese', value: '张三', mode: 'sanitized', forbidden: /[^\x00-\x7F]/ },
  { name: 'Non-English Characters - Arabic', value: 'محمد', mode: 'sanitized', forbidden: /[^\x00-\x7F]/ },
  { name: 'Non-English Characters - Hindi', value: 'राम', mode: 'sanitized', forbidden: /[^\x00-\x7F]/ },
...
const invalidPhoneScenarios = [
  ...
  { name: 'Emoji', value: '😊123456', mode: 'filtered', forbidden: /[\p{Emoji_Presentation}\u200d]/u },
  { name: 'Unicode Symbols', value: '☎123456', mode: 'filtered', forbidden: /[^\x00-\x7F]/ },
  { name: 'Non-English Digits - Arabic', value: '١٢٣٤٥٦٧٨٩٠', mode: 'filtered', forbidden: /[^\x00-\x7F]/ },
  { name: 'Non-English Digits - Hindi', value: '१२३४५६७८९०', mode: 'filtered', forbidden: /[^\x00-\x7F]/ },
...
const invalidPinCodeScenarios = [
  ...
  { name: 'Emoji', value: '😊1234', inputMethod: 'Paste', mode: 'filtered', forbidden: /[\p{Emoji_Presentation}\u200d]/u },
  { name: 'Unicode symbols', value: '★1234', inputMethod: 'Paste', mode: 'filtered', forbidden: /[^\x00-\x7F]/ },
  { name: 'Non-English digits - Fullwidth', value: '１２３４５', inputMethod: 'Paste', mode: 'filtered', forbidden: /[^\x00-\x7F]/ },
  { name: 'Non-English digits - Arabic', value: '١٢٣٤٥', inputMethod: 'Paste', mode: 'filtered', forbidden: /[^\x00-\x7F]/ },
...
const invalidBusinessListingEmailScenarios = [
  ...
  { name: 'Emoji', value: '😊@gmail.com', inputMethod: 'Paste' },
  { name: 'Unicode symbols', value: '★john@gmail.com', inputMethod: 'Paste' },
...
const pasteBusinessListingEmailScenarios = [
  ...
  { name: 'Paste emoji', value: '😊@gmail.com', inputMethod: 'Paste' },
...
const invalidUpdateBusinessDetailsWebUrlScenarios = [
  ...
  { name: 'Emoji', value: 'https://ðŸ˜Š.com', inputMethod: 'Typing', mode: 'reject' },
  ...
  { name: 'Non-English domain', value: 'https://à¤—à¥‚à¤—à¤².com', inputMethod: 'Typing', mode: 'reject' },
...
const pasteUpdateBusinessDetailsWebUrlScenarios = [
  ...
  { name: 'Paste URL with emoji', value: 'https://ðŸ˜Š.com', inputMethod: 'Paste', mode: 'reject' },
...
const updateBusinessDetailsPhoneScenarios = [
  ...
  { name: 'Emoji', value: '😊123456789', inputMethod: 'Paste', mode: 'red-alert-or-sanitized', forbidden: /[\p{Emoji_Presentation}\u200d]/u },
  { name: 'Unicode symbols', value: '☎123456789', inputMethod: 'Paste', mode: 'red-alert-or-sanitized', forbidden: /☎/ },
  { name: 'Non-English digits Arabic', value: '١٢٣٤٥٦٧٨٩٠', inputMethod: 'Paste', mode: 'red-alert-or-sanitized', forbidden: /[^\x00-\x7F]/ },
  { name: 'Non-English digits Full width', value: '１２３４５６７８９', inputMethod: 'Paste', mode: 'red-alert-or-sanitized', forbidden: /[^\x00-\x7F]/ },
  ...
  { name: 'Paste with emoji rejected', value: '123😊456789', inputMethod: 'Paste', mode: 'red-alert-or-sanitized', forbidden: /[\p{Emoji_Presentation}\u200d]/u },
...
const updateBusinessDetailsEmailScenarios = [
  ...
  { name: 'Emoji', value: '😊@company.com', inputMethod: 'Paste', mode: 'red-alert' },
  { name: 'Unicode symbols', value: '★business@company.com', inputMethod: 'Paste', mode: 'red-alert' },
  { name: 'SQL Injection', value: "' OR 1=1 --", inputMethod: 'Paste', mode: 'red-alert' },
  ...
  { name: 'Paste emoji', value: '😊@company.com', inputMethod: 'Paste', mode: 'red-alert' },
...
const fastStepAgentNameScenarios = [
  ...
  { name: 'Emoji', value: '😊John', inputMethod: 'Paste', mode: 'reject-or-sanitized', forbidden: /[\p{Emoji_Presentation}\u200d]/u },
  { name: 'Unicode Symbols', value: '★John', inputMethod: 'Paste', mode: 'reject-or-sanitized', forbidden: /[^\x00-\x7F]/ },
  ...
  { name: 'Paste emoji', value: '😊John', inputMethod: 'Paste', mode: 'reject-or-sanitized', forbidden: /[\p{Emoji_Presentation}\u200d]/u },
...
async function runLoginOtpValidationBeforeSignup({ page, loginPage, fastAgentDetailsPage, resilient }) {
  ...
  await resilient.run({
    name: 'Login OTP timer read verification',
    assert: async () => expect(await loginPage.getResendOtpTimerText()).toBeTruthy(),
    impact: ['OTP timer text was not readable before expiration validation.'],
    recoveryAction: 'Continue to wait for OTP expiration if the resend control is still available.',
    severity: 'VALIDATION',
  });

  OTPHelper.showTerminalMessage('Waiting for login OTP timer to expire.');
  await resilient.run({
    name: 'Login OTP timer expiration verification',
    assert: async () => loginPage.waitForOtpTimerToExpire(),
    continueOnFailure: false,
    impact: ['Expired OTP validation and resend OTP login cannot continue until timer reaches zero.'],
    recoveryAction: 'Stop pre-signup login flow and review OTP timer behavior.',
    severity: 'CRITICAL',
  });

  await loginPage.clearOtp();
  OTPHelper.showTerminalMessage(
    `Enter the expired login OTP for ${loginEmail}, then automation will validate the expired OTP popup.`
  );
  await loginPage.waitForLastOtpInputFilled();
  await loginPage.continueWithOtp();
  await resilient.run({
    name: 'Login expired OTP popup validation',
    assert: async () => loginPage.expectPopupMessage(otpData.messages.expiredOtp),
    impact: ['Expired OTP popup title/message/buttons could not be verified.'],
    recoveryAction: 'Close the popup and continue to resend OTP flow.',
    severity: 'VALIDATION',
  });
  await resilient.run({
    name: 'Login expired OTP popup close verification',
    assert: async () => loginPage.closePopup(),
    impact: ['Expired OTP popup may block resend OTP.'],
    recoveryAction: 'Continue to resend OTP after dismissing the popup.',
    severity: 'VALIDATION',
  });

  await resilient.run({
    name: 'Login resend OTP verification',
    assert: async () => loginPage.resendOtp(),
    continueOnFailure: false,
    impact: ['A fresh login OTP could not be requested.'],
    recoveryAction: 'Stop pre-signup login flow and review resend OTP behavior.',
    severity: 'CRITICAL',
  });
  await resilient.run({
    name: 'Login resend OTP timer restart verification',
    assert: async () => expect(await loginPage.getResendOtpTimerText()).not.toBe('Resend One Time Password'),
    impact: ['OTP timer may not have restarted after resend.'],
    recoveryAction: 'Continue because the tester can still enter the freshly sent OTP.',
    severity: 'VALIDATION',
  });
  await loginPage.closePopupIfVisible();
  await loginPage.clearOtp();

  OTPHelper.showTerminalMessage(`Enter the valid login OTP for ${loginEmail}.`);
  await loginPage.waitForLastOtpInputFilled();
  await loginPage.continueWithOtp();
...
async function runGoogleLoginBeforeSignup({ page, loginPage, fastAgentDetailsPage, resilient }) {
  ...
  await resilient.run({
    ...
    recoveryAction: 'Review Google account prompt state, credentials, and OAuth popup behavior.',
    severity: 'CRITICAL',
  });
...
async function resumeOnboardingInCurrentPage({ page, detailsUrl, resilient }) {
...
async function continueFromDraftAgentIfNeeded({
  page,
  detailsUrl,
  businessStepUrl,
  dashboardUrl,
...
async function runPersonalDetailsNameAutomation({ page, personalDetailsPage, detailsUrl, businessStepUrl }) {
...
async function runBusinessCategoryAutomation({ browser, page, businessStepUrl, dashboardUrl }) {
  const reporter = new AssertionImpactReporter(
...
async function runBusinessDetailsNameAutomation({ browser, page, businessStepUrl, dashboardUrl }) {
  const reporter = new AssertionImpactReporter(
...
async function runUpdateBusinessDetailsEmailScenario({
  ...
  await businessDetailsPage.manualEmailInput().blur();
  const continueEnabledBeforeClick = await businessDetailsPage.continueButton()
    .isEnabled({ timeout: timeouts.quickAction })
    .catch(() => false);
...
async function clickFinishAndWaitForFastStep({ page, businessStepUrl }) {
  ...
  await expect(page.locator('button:has-text("Finish")').first()).toBeEnabled({ timeout: timeouts.action });
  await page.locator('button:has-text("Finish")').first().click({ force: true });
...
async function completeRecommendedPlanAndStripeCheckout(page) {
  ...
  const chooseYourPlanButton = page.locator('button:has-text("Choose Your Plan")').first();
  if (await chooseYourPlanButton.isVisible({ timeout: timeouts.quickAction }).catch(() => false)) {
    await chooseYourPlanButton.click({ force: true });
    appendFastStepPostCallReport('Clicked Choose Your Plan');
  }

  await expect(page.locator(billingLocators.recommendedPlan.title).first()).toBeVisible({ timeout: timeouts.pageLoad });
  const planDetails = await collectRecommendedPlanDetails(page);

  updateStoredFinalBusinessDetails({
    recommendedPlanSelection: {
      pageUrl: page.url(),
      expectedPlans: expectedRecommendedPlans,
      verifiedPlans: planDetails,
      selectedPlan: 'Scaler',
      storedAt: new Date().toISOString(),
    },
  });
  appendFastStepPostCallReport('Recommended plan details verified and stored');

  const scalerCard = page
    .locator(`${billingLocators.recommendedPlan.planCard('SCALER')}, div:has-text("SCALER"):has-text("$299/month")`)
    .first();
  await expect(scalerCard).toBeVisible({ timeout: timeouts.pageLoad });
  await scalerCard.click({ force: true });
  appendFastStepPostCallReport('Selected Scaler plan');

  const continueButton = page
    .locator(`button:has-text("Selected plan"):has-text("Scaler"):has-text("Continue"), ${billingLocators.recommendedPlan.continueButton}`)
    .last();
  await expect(continueButton).toBeEnabled({ timeout: timeouts.action });
  await continueButton.scrollIntoViewIfNeeded().catch(() => {});
  await continueButton.click({ force: true });
  appendFastStepPostCallReport('Clicked Scaler Continue');
...
async function clickFastStepCallControl(page, labelPattern, description) {
  ...
  const locatorTarget = page.locator(selector).first();
  if (await locatorTarget.isVisible({ timeout: timeouts.shortAction }).catch(() => false)) {
    await locatorTarget.scrollIntoViewIfNeeded().catch(() => {});
    await locatorTarget.click({ force: true });
    appendFastStepPostCallReport(`Clicked ${description} using locator`);
    return;
  }
  ...
  const clicked = await page.evaluate((source) => {
    ...
    const candidates = Array.from(document.querySelectorAll('button, [role="button"], [class*="btnTheme"], p, span, div'))
      .filter(isVisible)
      .filter((element) => pattern.test(element.textContent || ''));

    const target =
      candidates
        .map((element) => element.closest('button, [role="button"], [class*="btnTheme"]') || element)
        .filter(isVisible)
        .filter((element, index, list) => list.indexOf(element) === index)
        .sort((a, b) => {
          const priority = (element) => {
            if (element.matches('button, [role="button"], [class*="btnTheme"]')) return 0;
            return 1;
          };
...
async function runFastStepPostCallFlow({ page, finalAgentName = 'Auto agent', skipRename = false } = {}) {
  ...
  await resilient.run({
    name: 'Fast Step post-call Go Live flow',
    assert: async () => {
      await expect(page).toHaveURL(/\/fast-step/, { timeout: timeouts.authRedirect });
      if (!skipRename) {
      ...
      let readyVisible = await fastStepReadyTitle(page)
        .isVisible({ timeout: timeouts.quickAction })
        .catch(() => false);

      if (!readyVisible) {
        await clickFastStepCallControl(page, /Make Web Call Now/, 'Make Web Call Now');
        appendFastStepPostCallReport('Waiting for Fast Step call to start');
        await expect
          .poll(
            async () =>
              ...
              (await page.getByText(/End Call|Ending/i).first().isVisible().catch(() => false)),
        ...
        if (
          (await page.locator(onboardingLocators.fastSetup.endCallButton).first().isVisible({ timeout: timeouts.shortAction }).catch(() => false)) ||
          (await page.getByText(/End Call|Ending/i).first().isVisible({ timeout: timeouts.shortAction }).catch(() => false))
        ) {
          await clickFastStepCallControl(page, /End Call|Ending/, 'End Call');
        }
      } else {
        appendFastStepPostCallReport('Agent ready screen was already displayed before Make Web Call step');
      }

      await expect(fastStepReadyTitle(page)).toBeVisible({
        timeout: timeouts.authRedirect,
      });
      activeAgentName = await getFastStepCurrentAgentName(page, activeAgentName);
      appendFastStepPostCallReport(`${activeAgentName} is ready screen displayed`);

      const goLiveButton = page.locator(onboardingLocators.fastSetup.goLiveButton).first();
      await goLiveButton.scrollIntoViewIfNeeded();
      await expect(goLiveButton).toBeVisible({ timeout: timeouts.pageLoad });

      await expect(goLiveButton).toBeEnabled({ timeout: timeouts.action });
      await goLiveButton.click({ force: true });
      appendFastStepPostCallReport('Clicked Go Live');
      ...
      updateStoredFinalBusinessDetails({
        fastStepPostCallFlow: {
          ...
          readyText: `${activeAgentName} is ready`,
          goLiveClicked: true,
          finalUrl: page.url(),
          reachedGoLiveSetup: currentPath(page) === '/go-live-setup',
    ...
    continueOnFailure: false,
    impact: [
      'Driver could not complete Fast Step call test, recommended plan checkout, Go Live setup, or Fast Agent Details tour.',
    ],
    recoveryAction:
      'Stop flow and inspect Fast Step post-call, recommended plan, Stripe checkout, Go Live setup, and Fast Agent Details tour flow.',
    severity: 'CRITICAL',
  });
}
...
test.describe('Signup', () => {
  test('signs up with email OTP, resumes onboarding, and completes Personal Details', async ({
    browser,
    page,
  }) => {
    ...
    const onboardingPath = await continueFromDraftAgentIfNeeded({
      page,
      detailsUrl,
      businessStepUrl,
      dashboardUrl,
    ...
    if (onboardingPath === '/details') {
      const personalDetailsPage = await resumeOnboardingInCurrentPage({
        page,
        detailsUrl,
        resilient: flowAssertions,
      });

      await runPersonalDetailsNameAutomation({
        page,
        personalDetailsPage,
        detailsUrl,
        businessStepUrl,
      });
    } else if (onboardingPath !== '/business-step') {
      throw new Error(`Unsupported onboarding resume path after signup: ${page.url()}`);
    }

    const businessDetailsPage = await runBusinessCategoryAutomation({
      browser,
      page,
      businessStepUrl,
      dashboardUrl,
    ...
    await runBusinessDetailsNameAutomation({
      browser,
