...
async function completeSignupViaEmailOtp({
  ...
  OTPHelper.showTerminalMessage(signupOtpData.terminalMessages.firstAttempt);

  await loginPage.waitForLastOtpInputFilled();
  await loginPage.continueWithOtp();

  const firstAttemptSucceeded = await resilient.run({
    name: 'Sign Up OTP first attempt verification',
    assert: async () => expect(page).toHaveURL(detailsUrl, { timeout: timeouts.authRedirect }),
    impact: [
      'First Sign Up OTP attempt did not redirect to Personal Details.',
      'A second OTP attempt is required.',
    ],
    recoveryAction: 'Validate error popup and allow one final OTP attempt.',
  });

  if (!firstAttemptSucceeded) {
    await resilient.run({
      name: 'Sign Up invalid OTP popup validation',
      assert: async () => {
      ...
      impact: ['Invalid OTP popup was not validated after failed Sign Up attempt.'],
      recoveryAction: 'Close popup and allow final OTP attempt.',
    });

    await resilient.run({
      name: 'Sign Up invalid OTP popup close verification',
      assert: async () => loginPage.closePopup(),
      impact: ['Invalid OTP popup may block final OTP attempt.'],
      recoveryAction: 'Continue with final OTP attempt.',
    });

    OTPHelper.showTerminalMessage(signupOtpData.terminalMessages.lastAttempt);

    await loginPage.clearOtp();
    await loginPage.waitForLastOtpInputFilled();
    await loginPage.continueWithOtp();

    const secondAttemptSucceeded = await resilient.run({
      name: 'Sign Up OTP final attempt verification',
      assert: async () => expect(page).toHaveURL(detailsUrl, { timeout: timeouts.authRedirect }),
      impact: [
        'Second Sign Up OTP attempt failed.',
        'Personal Details onboarding flow was not reached.',
      ],
      recoveryAction: 'End Sign Up automation gracefully after two failed OTP attempts.',
    });

    if (!secondAttemptSucceeded) {
      recordSkipped(reporter, 'Sign Up Personal Details verification', [
        'Only two OTP attempts are allowed.',
        'Sign Up OTP verification did not complete successfully.',
      ]);
      return false;
    }
  }
...
test.describe.skip('Login via Email OTP authentication setup', () => {
  ...
  test('validates OTP flow and stores authenticated session', async ({ page }) => {
    ...
    await test.step('Expired OTP validation', async () => {
      OTPHelper.showTerminalMessage('Negative OTP test cases completed.');

      await loginPage.clearOtp();

      const timerReadable = await resilient.run({
        name: 'OTP timer read verification',
        assert: async () => loginPage.getResendOtpTimerText(),
        impact: [
          'OTP expiration wait cannot be based on the visible timer.',
          'Expired OTP validation may be skipped.',
        ],
        recoveryAction: 'Skip expired OTP and resend validation if timer is unavailable.',
      });

      if (!timerReadable) {
        recordSkipped(reporter, 'Expired OTP and resend flow', [
          'OTP timer/resend control was unavailable.',
          'Expired OTP popup, resend OTP, successful login, and storage state generation were skipped.',
        ]);
        return;
      }

      const visibleTimerText = await loginPage.getResendOtpTimerText();
      OTPHelper.showTerminalMessage(`OTP expiration timer: ${visibleTimerText}`);
      OTPHelper.showTerminalMessage(`Visible timer text: ${visibleTimerText}`);
      OTPHelper.showTerminalMessage('Waiting for OTP timer to reach zero...');

      const timerExpired = await resilient.run({
        name: 'OTP timer expiration verification',
        assert: async () => loginPage.waitForOtpTimerToExpire(),
        impact: [
          'Expired OTP popup validation could not be executed.',
          'Resend OTP and successful login validation were skipped.',
        ],
        recoveryAction: 'Skip expired OTP/resend flow if the OTP timer does not expire.',
      });

      if (!timerExpired) {
        recordSkipped(reporter, 'Expired OTP and resend flow', [
          'OTP timer did not reach the resend state.',
          'Expired OTP popup, resend OTP, successful login, and storage state generation were skipped.',
        ]);
        return;
      }

      OTPHelper.showTerminalMessage('OTP has expired.');

      await loginPage.clearOtp();

      OTPHelper.showTerminalMessage('Please enter the expired OTP sent to your email.');
      ...
      await verifyNoAuthenticationBypass({
        ...
        reporter,
        criticalFailureHandler,
        scenario: 'expired OTP submission',
      });

      await resilient.run({
        name: 'Expired OTP popup validation',
        assert: async () => loginPage.expectPopupMessage(otpData.messages.expiredOtp),
        impact: [
          'Expired OTP popup content could not be validated.',
          'Expired OTP negative scenario report is incomplete.',
        ],
        recoveryAction: 'Attempt to close popup and continue with resend OTP flow.',
      });
      await resilient.run({
        name: 'Expired OTP popup close verification',
        assert: async () => loginPage.closePopup(),
        impact: ['Resend OTP may be blocked by the expired OTP popup.'],
        recoveryAction: 'Continue with resend OTP flow.',
      });
    });

    await test.step('Resend OTP and complete successful login', async () => {
      const resendClicked = await resilient.run({
        name: 'Resend OTP verification',
        assert: async () => loginPage.resendOtp(),
        impact: [
          'Valid OTP could not be requested.',
          'Successful authentication and storage state generation were skipped.',
        ],
        recoveryAction: 'Skip successful login if resend OTP fails.',
      });

      if (!resendClicked) return;

      await resilient.run({
        name: 'Resend OTP popup close verification',
        assert: async () => {
          await loginPage.expectPopupMessage(otpData.messages.otpSent);
          await loginPage.closePopup();
        },
        impact: ['Success popup may block valid OTP entry.'],
        recoveryAction: 'Close the resend success popup before valid OTP entry.',
      });
      await loginPage.clearOtp();

      OTPHelper.showTerminalMessage(`Please enter the valid OTP sent to ${otpData.email}.`);

      await loginPage.waitForLastOtpInputFilled();
      await loginPage.continueWithOtp();

      const firstValidOtpAttemptSucceeded = await resilient.run({
        name: 'Successful authentication URL verification',
        ...
        recoveryAction: 'Check whether a wrong OTP popup appeared and allow one final valid OTP attempt.',
      });

      if (!firstValidOtpAttemptSucceeded) {
        const invalidOtpPopupVisible = await resilient.run({
          name: 'Final OTP first attempt popup validation',
          assert: async () => loginPage.expectPopupMessage(otpData.messages.invalidOtp),
          impact: [
            'The wrong OTP popup could not be confirmed.',
            'The tester may not receive a clear final retry warning.',
          ],
          recoveryAction: 'Close the popup and request one final fresh OTP if the popup is present.',
        });

        if (invalidOtpPopupVisible) {
          await resilient.run({
            name: 'Final OTP first attempt popup close verification',
            assert: async () => loginPage.closePopup(),
            impact: ['The popup may block the final OTP retry.'],
            recoveryAction: 'Continue to resend OTP after closing the invalid OTP popup.',
          });
        } else {
          recordSkipped(reporter, 'Final OTP retry flow', [
            'The final OTP attempt did not redirect, but the invalid OTP popup was not visible.',
            'Resend OTP, logout, Google login, and storage state generation were skipped.',
          ]);
          return;
        }

        OTPHelper.showTerminalMessage(
          `Hey tester please be careful this is a last chance to continue with the automation further please enter correct otp resend to the ${otpData.email}`
        );

        const finalResendReady = await resilient.run({
          name: 'Final chance resend OTP verification',
          assert: async () => loginPage.resendOtpAndCloseSuccessPopup(otpData.messages.otpSent),
          impact: [
            'A fresh OTP could not be requested for the final retry.',
            'Successful authentication, logout, Google login, and storage state generation were skipped.',
          ],
          recoveryAction: 'Skip successful login if the final OTP resend flow fails.',
        });

        if (!finalResendReady) return;
